...
function signInstruction(instruction, deponentCode){
  ...
  var signData = null;
  if(instruction.deponentFrom == deponentCode){
    logger.trace('%s is transferer in ', deponentCode, helper.instruction2string(instruction) );
    signData = instruction.alamedaFrom;
  }else if(instruction.deponentTo == deponentCode){
    logger.trace('%s is receiver in ', deponentCode, helper.instruction2string(instruction) );
    signData = instruction.alamedaTo;
  } else {
    throw new Error('Cannot detect role in instruction (deponent code doesn\'t match: '+deponentCode+')');
  }

  if(!signData){
    logger.warn('Sign data is empty', helper.instruction2string(instruction) );
    return false;
  }
