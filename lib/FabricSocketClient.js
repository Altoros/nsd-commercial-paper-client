...
// util.inherits(FabricSocketClient, SocketClient);
...
function FabricSocketClient(endpoint){
  ...
  //   logger.info('got block no. %s', block.header.number);

  //   var instructionByEvent = helper.getInstructionsByEvent(block);
  //   Object.keys(instructionByEvent)
  //     .filter(type=>type!='default')
  //     .forEach(type=>{
  //       logger.debug('emit %s', 'event-'+type);
  //       self._socket.emit('event-'+type, instructionByEvent[type]);
  //     });
  // });
